...
```{r chn-obs, tab.cap="The method of time-series observations used for Snake River spring/summer Chinook Salmon abundance modeling differed across populations with the first observation beginning in 1980 and last observation in 2024."}
mod_inputs$dat %>%
  filter(!is.na(nosaij)) %>%
  ...
  arrange(mpg, pop) %>%
  kable(
    col.names = c('MPG', 'Population', 'Method', 'First', 'Last', 'n', 'min', 'max', 'average', 'sd'),
    booktabs = TRUE,
    digits = 0,
    label = NA
  ) %>%
