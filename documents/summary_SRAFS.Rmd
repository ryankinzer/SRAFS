---
title: "Snake Basin Anadromous Fish Status: Updated QET Analysis"
author:
- Jay Hesse:
...
```{r chn-obs, tab.cap="The method of time-series observations used for Snake River spring/summer Chinook Salmon abundance modeling differed across populations with the earliest beginning in 1980 and ending in 2024."}
mod_inputs$dat %>%
  filter(!is.na(nosaij)) %>%
  filter(spawningyear >= 1980) %>%
  group_by(mpg, pop, method) %>%
  summarise(min_year = min(spawningyear),
            max_year = max(spawningyear),
            n = n(),
  ...
  mutate(method = case_when(
    method == 2 ~ 'PIT-tag Observations',
    method == 1 ~ 'SGS and Weir Expansions')
  ) %>%
  arrange(mpg, pop) %>%
  kable(
    col.names = c('MPG', 'Population', 'Method', 'Begin', 'Last', 'n', 'min', 'max', 'average', 'sd'),
    booktabs = TRUE,
    digits = 0,
    label = NA
  ) %>%
...
```{r sth-obs, tab.cap="The method of time-series observations used for Snake River summer steelhead abundance modeling were primarily based on PIT-tag observations beginning in 2010 and ending in 2024. Three populations included a secondary time-series based on spawning ground survey expansions."}
mod_inputs$dat %>%
  filter(!is.na(nosaij)) %>%
  filter(spawningyear >= 1980) %>%
  group_by(mpg, pop, method) %>%
  summarise(min_year = min(spawningyear),
            ...
            sd_nosaij = sd(nosaij)) %>%
  mutate(method = case_when(
    method == 1 ~ 'PIT-tag Observations',
    method == 2 ~ 'SGS and Weir Expansions')
  ) %>%
  arrange(mpg, pop) %>%
  kable(
    col.names = c('MPG', 'Population', 'Method', 'Begin', 'Last', 'n', 'min', 'max', 'average', 'sd'),
